...
# 1. Алгоритмы (реализованы вручную)
...
def check_variant(data_dir: Path, variant: int) -> None:
    ...
    if actual != variant:
        raise SystemExit(
            ...
            f"Перегенерируйте: python scripts/generate_data.py --variant {variant} --only arrays")
...
def self_check() -> None:
    ...
    # ДОБАВЛЕННЫЕ ПРОВЕРКИ ИНВАРИАНТОВ (Пункт 6 задания методички)
    # Инвариант count_equal_pairs: на уникальных элементах пар всегда 0
    assert count_equal_pairs([1, 2, 3, 4, 5]) == 0
    # Инвариант array_max: если все элементы равны, максимум равен этому элементу
    assert array_max([42, 42, 42]) == 42
